...
- **Permission utilities** (`utils/permissions.js`): `hasRole()`, `canManageHiring()`, `canManageProjects()`, `isProjectManager()`, `canEditTask()`.
- **Guard component** (`components/features/auth/Guard.jsx`): Reusable wrapper for conditional rendering. Props: `role`, `roles`, `canManageHiring`, `canManageProjects`, `isProjectManager`, `canEditTask`.
- **usePermissions hook**: Programmatic access to permission checks.
- **Related auth UI paths**:
  - `components/features/auth/ProtectedRoute.jsx`
  - `components/layout/Navbar.jsx` (consumes Guard)

### Frontend Component Layout (Post-Reorg)
- Shared UI primitives live in `components/ui/` (e.g., `Spinner.jsx`, `StatCard.jsx`, `DashboardChartRegistry.jsx`).
- Domain components live in `components/features/<domain>/`:
  ...
  - `components/features/auth/Guard.jsx`

### Common Import Targets (Post-Reorg)
- Use these canonical imports in pages/components:
  - `Spinner` -> `components/ui/Spinner.jsx`
  - `StatCard` -> `components/ui/StatCard.jsx`
  - `DashboardChart` -> `components/ui/DashboardChartRegistry.jsx`
  - `ChatWindow` -> `components/features/chat/ChatWindow.jsx`
  - `Guard` / `usePermissions` -> `components/features/auth/Guard.jsx`
...
- Project routes include both scrum timeline and table variants:
  - `/projects/:pk/scrum`
  - `/projects/:pk/scrum2`
